...
['history -d',                                    '# remove a line from shell history (i.e. password); ~/.bash_history | ~/.config/fish/fish_history'],
...
['find . -type f ! -name "*~" -exec grep -nH -e "String" {} +',                 '# emacs find - exclude backup files'],
...
['grep -B 2 -A 4 "String" / grep --before-context=2 --after-context=4 "String"','# search for "String" and print 2/4 lines before/after matching line'],
['grep -lir "String" *',                                                        '# print only file-names'],
['grep "String" * | cut -f1 -d:',                                               '# print only filenames of the files containing "String"'],
['grep -i -n "String" *',                                                       '# print line numbers'],
['grep -r "String" --include=*.txt .',                                          '# find: search for "String" in *.txt'],
['find . -type f -name "*.txt" -print0 | xargs -0 grep -l "String"',            '# search for "String" in *.txt files (with spaces in filenames)'],
['grep --exclude=.git',                                                         '# '],
['grep --exclude=.git -lir "String" *.properties',                              '# '],
['grep -lir "String" $(find . -name *.properties)',                             '# '],
['grep -lir "String" $(find . -name *.properties -and -not -name .git/)',       '# do not search in the .git directory'],
...
['cvs log file.ext',                             '# version and tags'],
['cvs status -v file.ext',                       '# version and tags'],
['cvs -d cvs -t -d :pserver:faizal@localhost:/myrepos ci -m "test" -l "src/foo/Foo.ext"', '# debug and trace info'],
