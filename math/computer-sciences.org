...
* Math Structures:
** Semigroup (S, *): A set S with an associative binary operation: x * y
   Generalized (or weaker) form of a Group; preserves only associativity and closure under the binary operation from the axioms defining a group
   S - non-empty set
   * - associative operation: (x * y) * z = x * (y * z)

** Monoid (S, *, e); Also a Category 'many' morphisms and 'few' (only one) object M
   Group withouth inverse elements
   S - non-empty set
   * - associative operation: (x * y) * z = x * (y * z)
   e - neutral element of S (identity)

** Group (S, *, e): A set G of elements with a 'multiplication' operation (formalization of symetry concept)
   S - non-empty set
       every element of S has an inverse (dual obj):
       x-inverse: x * x-inverse = x-inverse * x = e
   * - associative operation: (x * y) * z = x * (y * z)
       does not need to be commutative (x * y = y * x)
   e - neutral element of S (identity)

   (i.e. a Category with one object; every morphism is an isomorphism)
   closure: a, b, a * b - all there elems must be membembers of the same group

** Cyclic Group: generated by one element
** Ring (S, +, *) - Okruh
** PreOrder (S, ≤): a nonempty set A with a pre-ordering relation s:
   S - non-empty set
   ≤ - pre-ordering relation: must be
                   - reflexive: a ≤ a
                   - transitive: a ≤ b and b ≤ c then a ≤ c

** PoSet - Partialy Ordered Set (S, ≤); Also a Category with 'few' (only one) morphisms between any two objects and many objects
   S - no-nempty set
   ≤ - relation: must be
                   - reflexive: a ≤ a
                   - transitive: a ≤ b and b ≤ c then a ≤ c
...
* PT 3 - Proof Search, Verifications & Sequent Calculus
  Verifications are subsets of all possible proofs
  A↑ - A has a verification
  A↓ - A may be used
