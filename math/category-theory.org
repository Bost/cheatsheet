...
* Category Theory: abstract algebra of abstract functions: "The Arrows Count"
  Category C = (Obj, Hom, ◦, id)
   ...
   - ◦: associative function "(f ◦ g) ◦ h = f ◦ (g ◦ h)" for morphism composition
...
*** Natural Transformation: a way of/for comparing functors
   ...
   - Every polymorphic fn is a NaT: it is defined for every single type
     i.e. multiplication (Product) of all obj in a category. The same goes for the dual - the Sum.
...
*** TODO:
   - Topos, Subobject Classifier etc.: see Bartosz's blog
...
* Leslie Lamport: Thinking Above the Code https://www.youtube.com/watch?v=-4Yp3j_jk8Q&t
  Usefull models:
** functions: mapping input to output; set of ordered pairs; Operating Systems is not an in/out mapping; it runs forever (see sequence of states)
...
*** Theorem: intersection of behaviors satisfying:
**** liveness property L: a complete behavior is needed
**** safety property S: e.g. partial correctness
***** specification:
...
* Informal spec: e.g. pretty-printing can't be exactly specified
